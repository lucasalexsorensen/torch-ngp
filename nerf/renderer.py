...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    if det:
        u = torch.linspace(0.0 + 0.5 / n_samples, 1.0 - 0.5 / n_samples, steps=n_samples).to(
            weights.device
        )
...
class NeRFRenderer(nn.Module):
    ...
    def run(
        ...
        **kwargs,
        ...
        if bg_model is not None:
            background_density_outputs = bg_model.density(xyzs.reshape(-1, 3))
            for k, v in background_density_outputs.items():
                background_density_outputs[k] = v.view(N, num_steps, -1)
        ...
        if upsample_steps > 0:
            ...
            if bg_model is not None:
                new_background_density_outputs = bg_model.density(new_xyzs.reshape(-1, 3))
                for k, v in new_background_density_outputs.items():
                    new_background_density_outputs[k] = v.view(N, upsample_steps, -1)
            ...
            if bg_model is not None:
                for k in background_density_outputs:
                    tmp_output = torch.cat(
                        [background_density_outputs[k], new_background_density_outputs[k]], dim=1
                    )
                    background_density_outputs[k] = torch.gather(
                        tmp_output, dim=1, index=z_index.unsqueeze(-1).expand_as(tmp_output)
                    )
        ...
        if bg_model is not None:
            background_alphas = 1 - torch.exp(
                -deltas * self.density_scale * density_outputs["sigma"].squeeze(-1)
            )  # [N, T+t]
            background_alphas_shifted = torch.cat(
                [torch.ones_like(background_alphas[..., :1]), 1 - background_alphas + 1e-15],
                dim=-1,
            )  # [N, T+t+1]
            background_weights = (
                background_alphas * torch.cumprod(background_alphas_shifted, dim=-1)[..., :-1]
            )  # [N, T+t]
        ...
        if bg_model is not None:
            for k, v in background_density_outputs.items():
                background_density_outputs[k] = v.view(-1, v.shape[-1])

        rgbs = self.color(
            xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1), **density_outputs
        )
        rgbs = rgbs.view(N, -1, 3)  # [N, T+t, 3]

        if bg_model is not None:
            background_rgbs = bg_model.color(
                xyzs.reshape(-1, 3), dirs.reshape(-1, 3), **background_density_outputs
            )
            background_rgbs = background_rgbs.view(N, -1, 3)  # [N, T+t, 3]
        ...
        if bg_model is not None:
            weights_sum -= background_weights.sum(dim=-1)
            pass
        ...
        # calculate color
        image_terms = weights.unsqueeze(-1) * rgbs
        if bg_model is not None:
            image_terms -= background_weights.unsqueeze(-1) * background_rgbs

        image = torch.sum(image_terms, dim=-2)  # [N, 3], in [0, 1]
    ...
    def run_cuda(
        ...
        **kwargs,
        ...
        nears, fars = raymarching.near_far_from_aabb(
            rays_o, rays_d, self.aabb_train if self.training else self.aabb_infer
        )
        ...
        if self.training:
            ...
            if bg_model is not None:
                weights_sum_full = torch.zeros(N, dtype=dtype, device=device)
                weights_sum_bg = torch.zeros(N, dtype=dtype, device=device)
            else:
            ...
            while step < 1024:  # hard coded max step
                ...
                if bg_model is None:
                    ...
                    density_outputs_full = self.density(xyzs)
                    sigmas_full = density_outputs_full["sigma"]
                    density_outputs_bg = bg_model.density(xyzs)
                    sigmas_bg = density_outputs_bg["sigma"]
                    rgbs_full = self.color(xyzs, dirs, **density_outputs_full)
                    rgbs_bg = bg_model.color(xyzs, dirs, **density_outputs_bg)
                    raymarching.composite_rays(
                        n_alive,
                        n_step,
                        rays_alive[i % 2],
                    ...
                    # raymarching.composite_rays_disentangle(
                    #     n_alive,
                    #     n_step,
                    #     rays_alive[i % 2],
                    #     rays_t[i % 2],
        ...
        if bg_model is None:
            ...
            image = image + (1 - weights_sum_full).unsqueeze(-1) * bg_color
        image = image.view(*prefix, 3)
                ...
                for zi, zs in enumerate(Z):
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            cam_xyzs = cam_xyzs @ poses[head:tail, :3, :3].transpose(
                                1, 2
                            )  # [S, N, 3]
            ...
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    for cas in range(self.cascade):
                        ...
                        # query density
                        sigmas = (
                            self.density(cas_xyzs.to(tmp_grid.device))["sigma"]
                            .reshape(lx, ly, lz)
                            .detach()
                        )
                        ...
                        # assign
                        tmp_grid[
                            cas, xi * S : xi * S + lx, yi * S : yi * S + ly, zi * S : zi * S + lz
                        ] = sigmas
    ...
    def render(
        ...
        perturb=False,
        bg_model=None,
        force_torch_ray=False,
        **kwargs,
    ):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3]
        use_cuda_ray = self.cuda_ray and not force_torch_ray
        ...
        if staged and not use_cuda_ray:
            ...
            results = _run(
                rays_o, rays_d, bg_color=bg_color, perturb=perturb, bg_model=bg_model, **kwargs
            )
