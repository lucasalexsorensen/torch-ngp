...
class Trainer(object):
    def __init__(
        ...
        if self.workspace is not None:
            if self.use_checkpoint == "scratch":
                ...
                self.load_checkpoint(self.model, self.ckpt_path, model_only=True)
            elif self.use_checkpoint == "best":
                if os.path.exists(self.best_path):
                    self.log("[INFO] Loading best checkpoint ...")
                    self.load_checkpoint(self.model, self.ckpt_path, self.best_path)
                else:
                ...
                self.load_checkpoint(self.model, self.ckpt_path, self.use_checkpoint)
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(
            rays_o, rays_d, staged=False, bg_color=bg_color, perturb=True, **vars(self.opt)
        )
    ...
    def test_step(self, data, bg_color=None, perturb=False):
        ...
        outputs = self.model.render(
            ...
            bg_color=bg_color,
            perturb=perturb,
            bg_model=self.background_model,
            force_torch_ray=self.background_model is not None,
            **vars(self.opt),
        )
    ...
    def test(self, loader, save_path=None):
        ...
        if save_path is None:
            save_path = os.path.join(self.workspace or self.opt.full, "results")
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                cv2.imwrite(
                    path_depth, (preds_depth[0].detach().cpu().numpy() * 255).astype(np.uint8)
                )
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1):
        ...
        if downscale != 1:
            ...
            preds = (
                F.interpolate(preds.permute(0, 3, 1, 2), size=(H, W), mode="nearest")
                .permute(0, 2, 3, 1)
                .contiguous()
            )
    ...
    def load_checkpoint(self, model, ckpt_path: str, checkpoint=None, model_only=False):
        if checkpoint is None:
            checkpoint_list = sorted(glob.glob(f"{ckpt_path}/{self.name}_ep*.pth.tar"))
            if checkpoint_list:
