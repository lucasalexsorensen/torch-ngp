...
def rand_poses(
    size, device, radius=1, theta_range=[np.pi / 3, 2 * np.pi / 3], phi_range=[0, 2 * np.pi]
):
...
class NeRFDataset:
    def __init__(self, opt, device, type="train", downscale=1, n_test=10):
        ...
        if self.mode == "colmap" and type == "test":
            ...
            for f in tqdm.tqdm(frames, desc=f"Loading {type} data:"):
                ...
                mask_path = os.path.join(
                    ...
                    "dynamic_mask_"
                    + os.path.basename(f_path).replace(".JPG", ".png").replace(".jpg", ".png"),
                )
                ...
                if self.opt.masked:
                    if not os.path.exists(mask_path):
                        ...
                        mask = 1 - (
                            cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
                        )
        ...
        if self.preload:
            ...
            if self.images is not None:
                self.images = self.images.to(torch.half if self.fp16 else torch.float).to(
                    self.device
                )
            if self.masks is not None:
                self.masks = self.masks.to(torch.half if self.fp16 else torch.float).to(
                    self.device
                )
