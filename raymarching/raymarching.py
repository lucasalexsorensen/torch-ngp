...
class _composite_rays(Function):
    ...
    def forward(
        ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, weights_sum, depth, image
    ):
...
class _composite_rays_disentangle(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)  # need to cast sigmas & rgbs to float
    def forward(
        ctx,
        ...
        Args:
            ...
            rays_alive: int, [N], the alive rays' IDs in N (N >= n_alive, but we only use first n_alive)
            rays_t: float, [N], the alive rays' time, we only use the first n_alive.
            sigmas_full: float, [n_alive * n_step,]
            sigmas_bg: float, [n_alive * n_step,]
            rgbs_full: float, [n_alive * n_step, 3]
            rgbs_bg: float, [n_alive * n_step, 3]
            deltas: float, [n_alive * n_step, 2], all generated points' deltas (here we record two deltas, the first is for RGB, the second for depth).
        In-place Outputs:
            weights_sum_full: float, [N,], the alpha channel
            weights_sum_bg: float, [N,], the alpha channel
            depth: float, [N,], the depth value
            image: float, [N, 3], the RGB channel (after multiplying alpha!)
        """
        _backend.composite_rays_disentangle(
            n_alive,
            n_step,
            rays_alive,
            rays_t,
...
composite_rays_disentangle = _composite_rays_disentangle.apply
